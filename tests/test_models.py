...
class TestInstanceStartRequest:
    ...
    def test_extra_args(self):
        req = InstanceStartRequest(
            ...
            extra_args=["--tp", "2", "--mem-fraction-static", "0.7"],
        )
        assert req.extra_args == ["--tp", "2", "--mem-fraction-static", "0.7"]
...
# Backend parameter field names kept in the models
_BACKEND_PARAM_FIELDS = [
    "ctx_size", "n_gpu_layers", "n_parallel", "threads", "batch_size",
    "flash_attn", "tensor_parallel_size", "max_model_len",
...
class TestBackendFieldParity:
    ...
    def test_default_config_has_all_fields(self):
        ...
        from inferx.params import _PARAM_TO_CONFIG
        for param_field, cfg_field in _PARAM_TO_CONFIG.items():
            assert cfg_field in cfg_fields, f"DefaultConfig missing cfg_field '{cfg_field}' for param '{param_field}'"

    def test_params_registry_matches_models(self):
        """Verify _PARAM_TO_CONFIG covers all fields in BackendParams."""
        from inferx.params import _PARAM_TO_CONFIG
        from inferx.models import BackendParams
        registry_fields = set(_PARAM_TO_CONFIG.keys())
        # alias is per-request only, no default in config
        model_fields = set(BackendParams.model_fields.keys()) - {"alias"}
        missing = model_fields - registry_fields
        assert not missing, f"_PARAM_TO_CONFIG missing coverage for: {missing}"
