...
async def start_instance(body: InstanceStartRequest):
    try:
        ...
        audit_log("instance.start", "instance", info.id, model=body.model, backend=str(body.backend))
        return info
    except FileNotFoundError as e:
        audit_log("instance.start", "instance", body.model, success=False, error_message=str(e))
...
async def stop_instance(inst_id: str):
    ...
    if not ok:
        audit_log("instance.stop", "instance", inst_id, success=False, error_message="not found")
        raise HTTPException(404, f"Instance not found: {inst_id}")
    audit_log("instance.stop", "instance", inst_id)
    return {"success": True}
...
async def instance_error(inst_id: str):
    ...
    error_logs = mgr.get_instance_error_logs(inst_id)
...
async def add_instance_tags(inst_id: str, tags: dict[str, str]):
    ...
    if not inst:
        raise HTTPException(404, f"Instance not found: {inst_id}")
    mgr.update_instance_tags(inst_id, tags)
